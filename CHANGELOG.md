...
- **Balance Sheet page (قائمة المركز المالي)** — new standalone page at `/admin/cost-management/balance-sheet` showing all-time cumulative financial position: Assets (inventory value = qty × unit cost, accounts receivable from pending orders), Equity (share capital, capital contributed, cumulative net profit, less distributions paid), and a per-shareholder equity breakdown table with columns for share capital, cash contributed, retained earnings share, distributions, and net equity.
- **Cash Flow Statement** — added to P&L dashboard as a third tab showing: Operating (net profit), Financing (capital received + distributions paid), and NET CASH MOVEMENT with per-shareholder distribution detail.
