<x-admin::layouts>
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <form id="filter-form" method="GET" action="{{ route('admin.cost_management.report.index') }}" class="flex items-center gap-3 flex-wrap">
                ...
                @foreach ([
                    ...
                    <a href="{{ route('admin.cost_management.report.index', ['from' => $range[0], 'to' => $range[1]]) }}"
                       class="rounded-lg border px-3 py-1.5 text-xs font-medium
                              {{ $from === $range[0] && $to === $range[1] ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-gray-300 text-gray-600 hover:bg-gray-50' }}">
                        {{ $label }}
                    </a>
                @endforeach
            </div>
    ...
    <div id="summary-section" class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5">
            ...
            <div class="space-y-3">
                ...
                <a href="{{ route('admin.cost_management.expenses.index') }}"
                   class="flex items-center gap-3 rounded-lg border border-gray-200 p-3 hover:bg-gray-50">
                    <span class="text-2xl">🧾</span>
                    <div>
