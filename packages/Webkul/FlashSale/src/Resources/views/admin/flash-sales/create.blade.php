...
@section('content')
    ...
    <div class="mt-6 box-shadow rounded-xl bg-white dark:bg-gray-900 p-6 max-w-2xl">
        ...
        <form
            method="POST"
            action="{{ route('admin.marketing.flash-sales.store') }}"
        >
            @csrf
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.marketing.flash-sales.index') }}" class="secondary-button">Cancel</a>
            </div>
        </form>
    </div>
    ...
    @pushOnce('scripts')
        ...
        (function () {
            ...
            function fetchProducts(q) {
                fetch('{{ route('admin.catalog.products.search') }}?query=' + encodeURIComponent(q))
                    .then(r => r.json())
                    .then(data => {
