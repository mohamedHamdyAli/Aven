...
@section('content')
            ...
            <tbody>
                @forelse ($deliveries as $d)
                    <tr class="border-b dark:border-gray-700">
                        ...
                        <td class="px-6 py-4 font-mono text-xs">{{ $d->bosta_id ?? '—' }}</td>
                        <td class="px-6 py-4">
                            @if ($d->tracking_number)
                                <span class="font-mono font-bold">{{ $d->tracking_number }}</span>
                                <a
                                    href="https://bosta.co/track/{{ $d->tracking_number }}"
                                    target="_blank"
    ...
    @pushOnce('scripts')
        ...
        function createShipment(orderId, btn) {
            ...
            fetch(`{{ url('admin/bosta/orders') }}/${orderId}/create-shipment`, {
                method: 'POST',
